...
public abstract class WikipediaPage extends Indexable {
	...
	public static class ContextedLink {
		private String anchor;
		private String target;
		...
		public String getContext() {
			return String.format("%s\t%s\t%d\t%d",anchor,context,start,end);
		}
	}
	...
	/**
	 * 03 Feb 2015 - Tuan: Add the extraction of links together with their window
	 * */
	public List<ContextedLink> extractContextedLinks() {
		int start = 0;
		...
		while (true) {
			...
			cl.context = sb.toString().replace('\t', ' ');
			cl.start = start;
			cl.end = end;
			links.add(cl);
